...
for_loop:
	slt $t0, $s2, $s3      # $t0 = 1 if $s2<=$s3 (v[j] <= v[end])
	beq $t0, $zero, exit   # exit for_loop if j > end 
	
	lw $t3, 0($s2) 
        slt $t0, $t3, $s0      # $t0 = 1 if $t3<=$s0 (if v[j] <= pivot)
        beq $t0, $zero, skipif # skip if statement (i.e. go to skipif when v[j] > pivot or when $t0 = 0)
	           
        lw $t0, 0($s1)         # $t0 (temp) = v[i]
...
skipif:
	addi $s2, $s2, 4       # j++
        j for_loop             # jump to test of outer loop

exit:
        lw $t3, 0($s2)         # $t3 = v[j], and $s2 is address of v[j]                     
        lw $t0, 0($s1)         # $t0 (temp) = v[i]
       
        sw $t3, 0($s1)	       # v[i] = v[j]
        sw $t0, 0($s2)	       # v[j] = temp
        
        sub $s1, $s1, $a0      # change address of i back to int i
        srl $s1, $s1, 2
